...
Run once after installing diffusers, and again after any change to diffusers_patch/:
...
# Names that also need to appear in the top-level diffusers/__init__.py
_TOP_LEVEL_EXPORTS = ["PixelDiTModel"]
...
def register_in_pipelines_init(D):
    ...
    # -- _import_structure dict entry --
    if '_import_structure["pixeldit"]' in txt:
        # update existing entry (re-run case)
        txt = re.sub(
    ...
    if "from .pixeldit import" in txt:
        ...
        print("[2b] Updated 'from .pixeldit import' line in pipelines/__init__.py")
    else:
        txt = txt.replace(
            "        from .stable_diffusion_3 import (",
            new_import_line + "\n        from .stable_diffusion_3 import (",
        )
        print("[2b] Added 'from .pixeldit import' line in pipelines/__init__.py")
...
def register_in_diffusers_init(D):
    ...
    for name in _TOP_LEVEL_EXPORTS:
        if name in txt:
            print(f"[3] {name} already in diffusers/__init__.py")
            continue

        # add to _import_structure list near FluxTransformer2DModel
        txt = txt.replace(
            '"FluxTransformer2DModel",',
            f'"FluxTransformer2DModel",\n            "{name}",',
        )
        # add to the TYPE_CHECKING / eager import block
        txt = txt.replace(
            "            FluxTransformer2DModel,",
            f"            FluxTransformer2DModel,\n            {name},",
        )
        # add to the `from .pipelines import (` block
        txt = txt.replace(
...
def main():
    ...
    print("\nDone!")
    print('Test: python -c "from diffusers.pipelines.pixeldit import PixelDiTPipeline, PixelDiTModel, QwenEncoder, PixelDiTJointAttnProcessor; print(\'OK\')"')
